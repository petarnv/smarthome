...
public abstract class AbstractWatchService {
        ...
        if (StringUtils.isNotBlank(pathToWatch)) {
            ...
            try {
                ...
                if (watchSubDirectories()) {
                    ...
                    registerAllDirectoriesInternal(toWatch, registeredWatchKeys);
                } else {
    ...
    /**
     * This method walks a file tree and registers all directories to be watched
     * by the watch service.
     * <br />
     * <br />
     * It can be used from the implementations of {@link AbstractWatchService}
     * to register directories, created after the service initialization.<br />
     * In order to do that make sure that
     * {@link AbstractWatchQueueReader#isWatchingDirectoryChanges()} is
     * true and process the create events in {@link AbstractWatchQueueReader#processWatchEvent()}
     * <br />
     * This method should be called only if {@link #watchSubDirectories} is true
     *
     * @param toWatch the directory being watched by the watch service
     * @param registeredWatchKeys a mapping between the registered directories and their {@link WatchKey registration
     *            keys}.
     * @throws IOException
     */
    final protected void registerAllDirectoriesInternal(Path toWatch, final Map<WatchKey, Path> registeredWatchKeys)
            throws IOException {
