...
class ConfigDispatchTest extends OSGiTest {
    ...
    private def final LINE_SEPARATOR = "line.separator"
    ...
    @Test
    public void 'configuration is updated when configuration files are modified'(){

        def configDirectory = "$CONFIGURATION_WATCH_DIRECTORY/modified_conf"
        def servicesDirectory = "modified_services"
        def defaultConfigFilePath = "$configDirectory/default.cfg"
        def modifiedFileName = "modified.service.file.cfg"

        initialize(configDirectory, servicesDirectory, defaultConfigFilePath)

        def initialPid = "initial.file.pid"
        def initialProperty = "initial.file.property"
        def initialValue = "initial.file.value"

        File serviceFile = new File("$configDirectory/$servicesDirectory/$modifiedFileName")

        verifyValueOfConfigurationProperty(initialPid, initialProperty, initialValue)

        RandomAccessFile raf = new RandomAccessFile(serviceFile, "rw")
        def initialLength = raf.length()

        // we'll try to add another line in the file
        def pid = "modified.file.pid"
        def property = "modified.file.property"
        def value = "modified.file.value"

        try {
            serviceFile << "\n$pid:$property=$value"

            // Assert that the configuration for the newly added pid is updated.
            verifyValueOfConfigurationProperty(pid, property, value)

        } catch (IOException | FileNotFoundException e){

            fail("An exception $e was thrown, while modifying the file $serviceFile.getPath()")
            ...
            waitForAssert {
                assertThat "The initial file content cannot be recovered", raf.length(), is(initialLength)
            }
            // Assert that the configuration for the newly added pid is updated
            verifyNotExistingConfigurationProperty(pid, property)
    ...
    @Test
    public void 'configuration is updated when the whole content of a file is deleted'(){

        def configDirectory = "$CONFIGURATION_WATCH_DIRECTORY/deleted_content_conf"
        def servicesDirectory = "deleted_content_services"
        def defaultConfigFilePath = "$configDirectory/default.file.cfg"
        def modifiedFileName = "deleted.content.service.file.cfg"

        initialize(configDirectory, servicesDirectory, defaultConfigFilePath)

        def initialPid = "deleted.content.service.pid"
        def initialProperty = "deleted.content.service.property"
        def initialValue = "deleted.content.service.value"
        ...
        try{
            ...
            raf.setLength(0)
            waitForAssert {
                assertThat raf.length(), is(0L)
            }

            verifyNotExistingConfigurationProperty(initialPid, initialProperty)
            ...
            // recover the initial content of the file
            serviceFile << "$initialPid:$initialProperty=$initialValue"
            waitForAssert {
                assertFalse(raf.length()==0L)
            }
            // assert that the configuration is updated properly
            verifyValueOfConfigurationProperty(initialPid, initialProperty, initialValue)
    ...
    @Test
    public void 'configuration is updated when pid, property and value are changed'(){

        def configDirectory = "$CONFIGURATION_WATCH_DIRECTORY/modified_line_conf"
        def servicesDirectory = "modified_line_services"
        def defaultConfigFilePath = "$configDirectory/default.cfg"
        def modifiedFileName = "modified.line.service.file.cfg"

        initialize(configDirectory, servicesDirectory, defaultConfigFilePath)

        def initialPid = "modified.line.pid2"
        def initialProperty = "modified.line.property2"
        def initialValue = "modified.line.value2"

        def temporaryPid = "modified.line.pid1"
        def temporaryProperty = "modified.line.property1"
        def temporaryValue = "modified.line.value1"

        File serviceFile = new File("$configDirectory/$servicesDirectory/$modifiedFileName")

        // assert that the configurations for both pid-s are updated properly with the initial file content
        verifyValueOfConfigurationProperty(initialPid, initialProperty, initialValue)
        verifyNoPropertiesForConfiguration(temporaryPid)

        try {
            // try to modify the file by changing the pid, the property and the value
            serviceFile.setText(serviceFile.readLines().get(0).replaceAll('2', '1') + System.getProperty(LINE_SEPARATOR))

            verifyValueOfConfigurationProperty(temporaryPid, temporaryProperty, temporaryValue)
            verifyNotExistingConfigurationProperty(initialPid, initialProperty)
            ...
            // recover the initial content of the file
            serviceFile.setText(serviceFile.readLines().get(0).replaceAll('1', '2') + System.getProperty(LINE_SEPARATOR))

            // assert that the configurations are updated properly
            verifyValueOfConfigurationProperty(initialPid, initialProperty, initialValue)
            verifyNotExistingConfigurationProperty(temporaryPid, temporaryProperty)
        }
    }

    @Test
    public void 'configuration is updated when only property and value are changed'(){

        def configDirectory = "$CONFIGURATION_WATCH_DIRECTORY/modified_line_conf"
        def servicesDirectory = "modified_property_and_value_services"
        def defaultConfigFilePath = "$configDirectory/default.cfg"
        def modifiedFileName = "modified.property.and.value.service.file.cfg"

        initialize(configDirectory, servicesDirectory, defaultConfigFilePath)

        def pid = "modified.property.and.value.pid1"
        def initialProperty = "modified.property.and.value.property1"
        def initialValue = "modified.property.and.value.value1"

        def temporaryProperty = "modified.property.and.value.property2"
        def temporaryValue = "modified.property.and.value.value2"

        File serviceFile = new File("$configDirectory/$servicesDirectory/$modifiedFileName")

        //assert that the configurations for both pid-s are updated properly with the initial file content
        verifyValueOfConfigurationProperty(pid, initialProperty, initialValue)
        verifyNotExistingConfigurationProperty(pid, temporaryProperty)

        try {
            // try to modify the file by changing the property and the value
            serviceFile.setText(serviceFile.readLines().get(0).replace(initialProperty, temporaryProperty) + System.getProperty(LINE_SEPARATOR))
            serviceFile.setText(serviceFile.readLines().get(0).replace(initialValue, temporaryValue) + System.getProperty(LINE_SEPARATOR))

            //assertThat the configuration is updated properly
            verifyValueOfConfigurationProperty(pid, temporaryProperty, temporaryValue)
            verifyNotExistingConfigurationProperty(pid, initialProperty)
            ...
            // recover the initial content of the file
            serviceFile.setText(serviceFile.readLines().get(0).replace(temporaryProperty, initialProperty) + System.getProperty(LINE_SEPARATOR))
            serviceFile.setText(serviceFile.readLines().get(0).replace(temporaryValue, initialValue) + System.getProperty(LINE_SEPARATOR))
    ...
    @Test
    public void 'configuration is updated when only value is changed'(){

        def configDirectory = "$CONFIGURATION_WATCH_DIRECTORY/modified_line_conf"
        def servicesDirectory = "modified_value_services"
        def defaultConfigFilePath = "$configDirectory/default.cfg"
        def modifiedFileName = "modified.value.service.file.cfg"

        initialize(configDirectory, servicesDirectory, defaultConfigFilePath)

        def pid = "modified.value.pid1"
        def property = "modified.value.property1"
        def initialValue = "modified.value.value1"

        def temporaryValue = "modified.value.value2"

        // assert that the configuration is updated properly with the initial file content
        verifyValueOfConfigurationProperty(pid, property, initialValue)

        File serviceFile = new File("$configDirectory/$servicesDirectory/$modifiedFileName")

        try {
            // try to modify the file by changing only the value
            serviceFile.setText(serviceFile.readLines().get(0).replace(initialValue, temporaryValue) + System.getProperty(LINE_SEPARATOR))

            //assert that the configuration is updated properly
            verifyValueOfConfigurationProperty(pid, property, temporaryValue)
